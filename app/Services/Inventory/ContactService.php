...
class ContactService
{
    ...
    {
        $all_data = [];
        if (Str::substr($type, 0, 1) == 'S') {
            $all_data = [
                'rows' => Contact::where('type', 'Customer')->get()
            ];
        } elseif (Str::substr($type, 0, 1) == 'P') {
            $all_data = [
                'rows' => Contact::where('type', 'Vendor')->get()
            ];
        } elseif (Str::contains($type, 'index')) {
